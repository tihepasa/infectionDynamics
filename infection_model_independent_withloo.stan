...
transformed parameters {
  ...
  // compute pointwise (region & time) log-likelihood here
  ...
  vector[n_obs] log_lik;
  {
    ...
    for (t in 1:(T - 1)) {
      ...
      b_p .* x[t,,1] * beta_p + c_p .* z[t,,1] * gamma_p;
    }
    for(i in 1:n_obs) {
      // add to loglik
    ...
    for (t in 1:(T - 1)) {
      ...
      b_m .* x[t,,2] * beta_m + c_m .* z[t,,2] * gamma_m;
    }
    for(i in 1:n_obs) {
      // add to loglik
    ...
    for (t in 1:(T - 1)) {
      ...
      b_s .* x[t,,3] * beta_s + c_s .* z[t,,3] * gamma_s;
    }
    for(i in 1:n_obs) {
      // add to loglik
