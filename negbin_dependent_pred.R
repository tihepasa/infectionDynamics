...
fit <- readRDS("fit_negbin_dependent.rds")
#fit <- readRDS("fit_negbin_dependent_s.rds")
...
alpha_phi_p <- e$alpha_phi_p
alpha_phi_m <- e$alpha_phi_m
alpha_phi_s <- e$alpha_phi_s
...
for (t in 1:(par$T - 1)) {
  ...
  ## pertussis
  # expected values for all iterations and sites in current time point
  # in case of model without seasonal effect, remove s1/s2/s3
  # this is only temperary variable not to be saved
  temp_sim <- exp(a1 +
                    s1[, 1 + t %% 12] +
                    lam1 * tau1[, t] + 
                    b1[, 1] * coefs1[, , 1] * temp_obs_p1 + b1[, 2] * coefs1[, , 2] * p_per_temp +
                    b1[, 3] * coefs1[, , 3] * temp_obs_m1 + b1[, 4] * coefs1[, , 4] * p_mea_temp + 
                    b1[, 5] * coefs1[, , 5] * temp_obs_s1 + b1[, 6] * coefs1[, , 6] * p_sma_temp)
  
  # probability to observe at least one death
  size <- exp(c(alpha_phi_p) + phi_p)
  pr_temp_p[,, t] <- matrix(1 - pnbinom(0, mu = (temp_sim), size = size), nrow = Nsim, ncol = par$N, byrow = FALSE)
  ...
  temp_obs_p[par$missing[, t + 1] == 1, ] <- rnbinom(Nsim * n_missing,
                                                               mu = (temp_sim[, par$missing[, t + 1] == 1]),
                                                               size = (size[, par$missing[, t + 1] == 1])) > 0
  
  ## measles
  temp_sim <- exp(a2 +
                    s2[, 1 + t %% 12] +
                    lam2 * tau2[, t] + 
                    b2[, 1] * coefs2[, , 1] * temp_obs_p1 + b2[, 2] * coefs2[, , 2] * p_per_temp +
                    b2[, 3] * coefs2[, , 3] * temp_obs_m1 + b2[, 4] * coefs2[, , 4] * p_mea_temp + 
                    b2[, 5] * coefs2[, , 5] * temp_obs_s1 + b2[, 6] * coefs2[, , 6] * p_sma_temp)
  
  size <- exp(c(alpha_phi_m) + phi_m)
  pr_temp_m[,, t] <- matrix(1 - pnbinom(0, mu = (temp_sim), size = size), nrow = Nsim, ncol = par$N, byrow = FALSE)
  
  pr_time_m[t, ] <- descript(rowMeans(pr_temp_m[,, t]))
  
  temp_obs_m <- matrix(obs_mea[, t + 1], ncol = Nsim, nrow = par$N, byrow = FALSE)
  temp_obs_m[par$missing[, t + 1] == 1, ] <- rnbinom(Nsim * n_missing,
                                                               mu = (temp_sim[, par$missing[, t + 1] == 1]),
                                                               size = (size[, par$missing[, t + 1] == 1])) > 0
  ...
  temp_sim <- exp(a3 +
                    s3[, 1 + t %% 12] +
                    lam3 * tau3[, t] + 
                    b3[, 1] * coefs3[, , 1] * temp_obs_p1 + b3[, 2] * coefs3[, , 2] * p_per_temp +
                    b3[, 3] * coefs3[, , 3] * temp_obs_m1 + b3[, 4] * coefs3[, , 4] * p_mea_temp + 
                    b3[, 5] * coefs3[, , 5] * temp_obs_s1 + b3[, 6] * coefs3[, , 6] * p_sma_temp)
  
  size <- exp(c(alpha_phi_s) + phi_s)
  tn_temp_s[,, t] <- matrix(1 - pnbinom(0, mu = (temp_sim), size = size), nrow = Nsim, ncol = par$N, byrow = FALSE)
  
  pr_time_s[t, ] <- descript(rowMeans(pr_temp_s[,, t]))
  
  temp_obs_s <- matrix(obs_sma[, t + 1], ncol = Nsim, nrow = par$N, byrow = FALSE)
  temp_obs_s[par$missing[, t + 1] == 1, ] <- rnbinom(Nsim * n_missing,
                                                               mu = (temp_sim[, par$missing[, t + 1] == 1]),
                                                               size = (size[, par$missing[, t + 1] == 1])) > 0
